...
{
    writeln (enumerateExtensions(theAllocator()));
    ...
    createInstance(
        CreateInstanceInfo(
            ...
            ["GAPISDLWindowInfo", "GAPIVideoDecode"]
        ),
        theAllocator(),
        instance
    );

    writeln ("03 - Vertex Layout");

    writeln ("Find best device...");
    PhysDevice pBestDevice = findBestDevice(instance);
    Device bestDevice;

    writeln("Best device has find: ", pBestDevice.getProperties().deviceName);
    writeln("Create handle device...");

    QueueFamilyProperties[] vq = pBestDevice.getQueueFamilyProperties();
    ...
    pBestDevice.extensions.writeln;

    bestDevice = instance.createDevice(
        pBestDevice, DeviceCreateInfo(
            cq,
            [
                ValidationLayerInfo(
        ...
        Command(CommandType.createPipeline, CmdCreatePipeline(
            ...
            VertexInputBindingDescription(
                ...
                [
                    ...
                    VertexInputAttributeDescription(
                        1, VertexAttributeFormat.Float, 4, 2 * float.sizeof
                    )
                ]
            ),
